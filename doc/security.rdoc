...
or by C extensions calling `ID2SYM`. In those cases, you shouldn't use user input.
Since immortal symbols can't be garbage collected, using user input for large
immortal symbols can cause a DDoS vulnerability.

Other calls that used to be vulnerable such as +send+, +instance_variable_get+
or +_set+, +const_get+ or +_set+, etc. are no longer a threath.
